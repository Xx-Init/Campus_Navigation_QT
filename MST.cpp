...
void MST:: dfsMST(int u, int fa){
    ...
    for(int i = head[u]; i; i = e2[i].nxt){
        int v = e2[i].to;
        if(vis[v]) continue;
        uzzi->listWidget->addItem(QString::fromStdString(int_to_places[u])+' '+QString::fromStdString(int_to_places[v])+' '+QString::number(e2[i].dis));
        dfsMST(v, u);
    }   
}
void MST:: printMST(){
    uzzi->listWidget->addItem("[---Begin---] Outputs the minimum spanning tree");
    uzzi->listWidget->addItem("There are "+QString::number(numOfPlace)+" on the minimum spanning tree");
    for(auto i: int_to_places){
        uzzi->listWidget->addItem(QString::fromStdString(i.second));
    }
    vis = new int[totNum+1];
    fill(vis, vis+totNum+2, 0);
    dfsMST(e1[0].u, 0);
   uzzi->listWidget->addItem("[---Finish---] Outputs the minimum spanning tree");
